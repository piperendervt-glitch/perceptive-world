...
**CL2: writer.md に I-D8/I-D9 に沿う執筆指針を追加。詩的密度は保ち、曖昧さは謎に集め、骨は明快に、超常は主観/客観を明示。**

### DONE 条件（CL2）

- [ ] writer.md に、詩性を残しつつ曖昧さを登録済みの謎に限定する指針が明記される。
- [ ] 物語の骨（行動・場所・因果・誰が何を）は指示対象を一義に保つ指針（指示語/目的語の省略回避・字義破壊の比喩回避）。
- [ ] 超常は主観/客観を明示する指針（I-D9・invariants 適合）。
- [ ] 「濃く書く」と「ぼかす」を混同しない＝曖昧さの再配分であることを明記。
- [ ] QA（CL3）には手を付けない（先回りしない）。
- [ ] git commit 後、`vCL.2` タグが打たれている。
