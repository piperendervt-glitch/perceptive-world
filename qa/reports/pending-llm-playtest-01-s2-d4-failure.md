# pending-llm-playtest-01-s2-d4-failure — 意味判断の小片パック

> qa_run.py が CODE 段通過後に生成。**外部 API は使わない。**
> Claude Code セッションが各小片を読み、重大度を記入する。
> 記入後、判定を qa/reports/consistency-playtest-01-s2-d4-failure.md に統合する（editor 相当）。
...
snippet:
```
石は、何も渡してこない。 （残り MP 1。何も読めなかった） 背後で、湯の音が止んだ。
```
meta: {'sentence': '（残り MP 1。何も読めなかった）', 'categories': ['mp', 'number'], 'matched': ['MP', '1'], 'attribution_hint': 'monologue', 'sentence_index': 10}
...
息だけで、いちばん小さな灯火を。
```
meta: {'delta_index': 0, 'char': 'H', 'cause': '灯火を灯したが刻印は読めなかった', 'change': {'mp.cur': -1}, 'is_mp_change': True, 'is_hp_change': False, 'matched_keywords': ['灯火', '灯'], 'score': 3}
判定: ____（記入）    理由: ____（記入）
...
snippet:
```
指先に、青白い点がともる。 その点を、石の刻印に寄せる。 彫りの一番深い線は、光をはね返すばかりだった。
```
...
snippet:
```
彫りの一番深い線は、光をはね返すばかりだった。 起き上がらない——H の目にすら、揺れて閉じていくように見えた。 読もうとするほど、面が硬くなる。
```
meta: {'sentence': '起き上がらない——H の目にすら、揺れて閉じていくように見えた。', 'kinds': ['supernatural'], 'matched': ['揺れ', '起き上が'], 'has_subjective_marker': True, 'sentence_index': 7, 'ask': 'この曖昧さは open_loops の登録謎に直接繋がるか（繋がる=通過 / 繋がらない=Warning）。超常は主観/客観の帰属が読めるか（I-D9）も見る。'}
判定: ____（記入）    理由: ____（記入）
