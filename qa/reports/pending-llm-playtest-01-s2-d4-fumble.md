# pending-llm-playtest-01-s2-d4-fumble — 意味判断の小片パック

> qa_run.py が CODE 段通過後に生成。**外部 API は使わない。**
> Claude Code セッションが各小片を読み、重大度を記入する。
> 記入後、判定を qa/reports/consistency-playtest-01-s2-d4-fumble.md に統合する（editor 相当）。
...
snippet:
```
読むどころではない。 （残り MP 1。制御を、失った） 女が振り返った。
```
meta: {'sentence': '（残り MP 1。制御を、失った）', 'categories': ['mp', 'number'], 'matched': ['MP', '1'], 'attribution_hint': 'monologue', 'sentence_index': 10}
...
息だけで、いちばん小さな灯火を。
```
meta: {'delta_index': 0, 'char': 'H', 'cause': '灯火が暴れたが刻印は読めなかった', 'change': {'mp.cur': -1}, 'is_mp_change': True, 'is_hp_change': False, 'matched_keywords': ['灯火'], 'score': 2}
判定: ____（記入）    理由: ____（記入）
...
snippet:
```
指先に、青白い点がともる。 その点を、石の刻印に寄せる。 息だけの灯火が、爆ぜた。
```
...
snippet:
```
その点を、石の刻印に寄せる。 息だけの灯火が、爆ぜた。 極小のはずの点が手のひらで膨らみ、青白く小屋の壁まで舐めた。
```
meta: {'sentence': '息だけの灯火が、爆ぜた。', 'kinds': ['supernatural'], 'matched': ['灯火'], 'has_subjective_marker': False, 'sentence_index': 6, 'ask': 'この曖昧さは open_loops の登録謎に直接繋がるか（繋がる=通過 / 繋がらない=Warning）。超常は主観/客観の帰属が読めるか（I-D9）も見る。'}
判定: ____（記入）    理由: ____（記入）

### 候補 5
snippet:
```
極小のはずの点が手のひらで膨らみ、青白く小屋の壁まで舐めた。 刻印の線は、光を嫌って底へ沈む——ように、H の目には映った。 読むどころではない。
```
meta: {'sentence': '刻印の線は、光を嫌って底へ沈む——ように、H の目には映った。', 'kinds': ['supernatural'], 'matched': ['刻印'], 'has_subjective_marker': True, 'sentence_index': 8, 'ask': 'この曖昧さは open_loops の登録謎に直接繋がるか（繋がる=通過 / 繋がらない=Warning）。超常は主観/客観の帰属が読めるか（I-D9）も見る。'}
判定: ____（記入）    理由: ____（記入）
